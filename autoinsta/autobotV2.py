            ...
            while True:
                if iframe is None:
                    ...
                    try:
                        ...
                        logging.info("[-] ReCapchat not Found 92")
        else:
            logging.info("[-] ReCapchat not Found 94")
            result = True
        return result
                ...
                for response_part in msg_data:
                    if isinstance(response_part, tuple):
                        ...
                        try:
                            ...
                            resp = subject.split(" ")[0]#chat_with_gpt(prompt, self.api_url)
                            confirmation_code = resp
                ...
                for response_part in msg_data:
                    if isinstance(response_part, tuple):
                        ...
                        try:
                            ...
                            resp = subject.split(" ")[0]#chat_with_gpt(prompt)
                            confirmation_code = resp
                            print(f"Código de confirmación: {confirmation_code}")
                            break
                            #if resp["check"]:
    ...
    def perform_button_click_and_input(self, max_timeout=120):
        ...
        while True:
            try:
                ...
                except Exception as ens:
                    print("Error save session")
                try:
                    self.notification(self.driver)
                except Exception as ens:
                    print("Error save notifications 401")
...
class ManageInsta:
    ...
    def _webdriver(self, proxy_ip, port):
        #if platform.system() == "Windows":
        return self._driver_chrome(proxy_ip, port)
        #else:
    ...
    def _driver_chrome(self, proxy_ip, port):
    ...
    def create_account(self, _driver:webdriver.Chrome):
        try:
            status = True
            block = False
            check_account = True
            ...
            for b in _driver.find_elements_by_xpath("//button[@type='submit']"):
                if "Registrarte" == str(b.text).strip() or "Siguiente" == str(b.text).strip() or "Sign up" in str(b.text).strip() or "Next" in str(b.text).strip():
                    ...
                    break
            time.sleep(3)
            if "Another account is using the same email." in _driver.page_source or "Este nombre de usuario no está disponible. Prueba otro." in _driver.page_source or "This username isn't available. Please try another." in _driver.page_source or "Otra cuenta usa el mismo correo electrónico." in _driver.page_source or "Los nombres de usuario solo pueden contener letras, números, guiones bajos y puntos." in _driver.page_source:
                logging.info(f"Este nombre de usuario no está disponible. Prueba otro: {self._email}")
                return status, block, check_account
            ...
            block = False
            check_account = True
        #print(status, block, check_account)
        return status, block, check_account
